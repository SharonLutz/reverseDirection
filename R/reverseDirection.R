...
TwoSample=F,sig.level=0.05,SEED=1,plot.pdf=T,plot.name="reverseDirection.pdf"){
...
if(length(MAF)!=1| min(MAF)<0 | max(MAF)>1){stop("MAF must be a single value greater than 0 and less than 1")}
...
if(length(gamma0)!=1){stop("length(gamma0) must equal 1")}
if(length(gammaG)!=1){stop("length(gammaG) must equal 1")}
if(length(varX)!=1){stop("length(varX) must equal 1")}

if(length(delta0)!=1){stop("length(delta0) must equal 1")}
if(length(deltaX)!=1){stop("length(deltaX) must equal 1")}
if(length(varME)!=1){stop("length(varME) must equal 1")}

if(length(beta0)!=1){stop("length() must equal 1")}
if(length(betaG)!=1){stop("length() must equal 1")}
...
if(plot.pdf!=T & plot.pdf!=F){stop("plot.pdf must equal True or False. Please not that R is case sensitive.")}

if(TwoSample==T|TwoSample=="T"|TwoSample=="t"|TwoSample=="True"|TwoSample=="true"){TwoSample<-T}  
if(TwoSample==F|TwoSample=="F"|TwoSample=="f"|TwoSample=="False"|TwoSample=="false"){TwoSample<-F} 
if(TwoSample!=T & TwoSample!=F){stop("TwoSample must equal True or False. Please not that R is case sensitive.")}
...
if(pleiotropy!=T & pleiotropy!=F){stop("pleiotropy must equal True or False. Please not that R is case sensitive.")}
...
if(measurementError!=T & measurementError!=F){stop("measurementError must equal True or False. Please not that R is case sensitive.")}
...
################################################################################                            
        ...
        if(TwoSample==T){
        	# Generate G1 from sample 1 (1 SNP)
		G <- rbinom(n,2,MAF)
        # Generate X (exposure/ intermediate phenotype)
        Xtrue <- rnorm(n,(gamma0 + gammaG*G),sqrt(varX))
		if(measurementError==T){X<-rnorm(n,(delta0+deltaX*Xtrue),sqrt(varME))}
		...
		# Generate G2 from sample 2 (1 SNP)
		G2 <- rbinom(n,2,MAF)
		# Generate Y (outcome)
        if(pleiotropy==T){Y <- rnorm(n,(beta0 + betaX[bX]*Xtrue +betaG*G2),sqrt(varY))}
        if(pleiotropy==F){Y <- rnorm(n,(beta0 + betaX[bX]*Xtrue),sqrt(varY))}
        
        matR[bX,"corGY"]<-matR[bX,"corGY"]+cor(G2,Y)
        matR[bX,"corGX"]<-matR[bX,"corGX"]+cor(G,X)
        matR[bX,"corXY"]<-matR[bX,"corXY"]+cor(X,Y)
        }
...
pSteiger<-Stest$p
 ...
 if(TwoSample==T){
#william test of correlation from Steiger paper
Stest<-r.test(n, r12 = abs(cor(G,X)), r34 = abs(cor(G2,Y)),twotailed = TRUE)
Z<-Stest$z
pSteiger<-Stest$p
}
